...
#     pass
    ...
    mean = np.mean(y_arr)
    numerator = np.sum(np.square(y_arr - estimated_arr))
    denominator = np.sum(np.square(y_arr - mean))
...
#     pass
    for model in models:
        ...
        plt.title('Degree of this model is {} \n R-squared for this model is {}'.format(len(model)-1,r_squared(y,np.polyval(model,x))))
